...
def Chaikin( X0 ) :
    ...
    for i in range(n):
        X1[2*i] = 3/4*X0[i] + 1/4*X0[(i+1)%n]
        X1[2*i+1] = 1/4*X0[i] + 3/4*X0[(i+1)%n]
...
def FourPoint( X0, w ) :
    ...
    for i in range(n):
        X1[2*i] = X0[i]
        X1[2*i+1] = (-w*X0[(i-1)%n] + (1/2+w)*X0[i] + (1/2+w)*X0[(i+1)%n] - w*X0[(i+2)%n])/2
    return X1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 :
        ...
        dataname = "simple" # [simple,infinity,bone,bunny]
    ###############################
